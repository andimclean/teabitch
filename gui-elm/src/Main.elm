...
type Route
    = HomeRoute
    | RoomRoute String String
    | MemberRoute String String String
...
matchers =
    oneOf
        ...
        , UrlParser.map RoomRoute (string </> string)
        , UrlParser.map MemberRoute (string </> string </> string)
        ]
...
init location =
    ...
    in
        ( Model Nothing [] [] Nothing name room NotConnected 0 False [] Nothing host Desktop "elm"
        , Cmd.none
        )


getVersionType : Route -> String
getVersionType route =
    case route of
        HomeRoute ->
            "elm"
    ...
    case msg of
        ...
        OnLocationChange location ->
            let
                ...
                host =
                    location.protocol ++ "//" ++ location.host ++ "/" ++ version ++ "/"

                newModel =
                    { model | room = room, name = name, me = Just me, host = host, version = version }
...
makeUrlCmd : { a | room : String, version : String, name : String } -> Cmd msg
makeUrlCmd model =
    if (canJoin model) then
        Navigation.newUrl <| "/" ++ model.version ++ "/" ++ model.room ++ "/" ++ model.name
    else
        Cmd.none
